...
FullCalendar.Interaction = (function (exports, core, internal) {
    ...
    processFirstCoord (ev) {
      ...
      if (initialHit) {
        if (this.useSubjectCenter && subjectRect) {
          const slicedSubjectRect = internal.intersectRects(subjectRect, initialHit.rect)
  ...
  class DateClicking extends internal.Interaction {
    constructor (settings) {
      ...
      const hitDragging = this.hitDragging = new HitDragging(this.dragging, internal.interactionSettingsToStore(settings))
  ...
  class EventDragging extends internal.Interaction {
    constructor (settings) {
      ...
      this.handlePointerDown = (ev) => {
        ...
        const subjectSeg = this.subjectSeg = internal.getElSeg(ev.subjectEl)
      ...
      this.handleDragEnd = (ev) => {
        if (this.isDragging) {
          ...
          const eventApi = new internal.EventImpl(initialContext, eventDef, eventInstance)
          const relevantEvents = this.relevantEvents
          ...
          if (validMutation) {
            ...
            if (receivingContext === initialContext) {
              const updatedEventApi = new internal.EventImpl(initialContext, mutatedRelevantEvents.defs[eventDef.defId], eventInstance ? mutatedRelevantEvents.instances[eventInstance.instanceId] : null)
              ...
              const addedEventApi = new internal.EventImpl(receivingContext, addedEventDef, addedEventInstance)
      ...
      dragging.touchScrollAllowed = false
      dragging.autoScroller.isEnabled = options.dragScroll
      const hitDragging = this.hitDragging = new HitDragging(this.dragging, internal.interactionSettingsStore)
  ...
  function computeEventMutation (hit0, hit1, massagers) {
    ...
    const delta = internal.diffDates(date0, date1, hit0.context.dateEnv, hit0.componentId === hit1.componentId
      ? hit0.largeUnit
      : null)
  ...
  class EventResizing extends internal.Interaction {
    constructor (settings) {
      ...
      this.handlePointerDown = (ev) => {
        ...
        const seg = internal.getElSeg(segEl)
      ...
      this.handleDragEnd = (ev) => {
        ...
        const eventDef = this.eventRange.def
        const eventInstance = this.eventRange.instance
        const eventApi = new internal.EventImpl(context, eventDef, eventInstance)
        const relevantEvents = this.relevantEvents
        const mutatedRelevantEvents = this.mutatedRelevantEvents
        ...
        if (this.validMutation) {
          const updatedEventApi = new internal.EventImpl(context, mutatedRelevantEvents.defs[eventDef.defId], eventInstance ? mutatedRelevantEvents.instances[eventInstance.instanceId] : null)
          context.dispatch({
            type: 'MERGE_EVENTS',
            eventStore: mutatedRelevantEvents
          })
  ...
  function computeMutation (hit0, hit1, isFromStart, instanceRange) {
    ...
    const delta = internal.diffDates(date0, date1, dateEnv, hit0.largeUnit)
    if (isFromStart) {
  ...
  class UnselectAuto {
    constructor (context) {
      ...
      this.onDocumentPointerDown = (pev) => {
        ...
        const downEl = internal.getEventTargetViaRoot(pev.origEvent)
    ...
    constructor (dragging, suppliedDragMeta) {
      ...
      this.handleDragEnd = (pev) => {
        ...
        if (receivingContext && droppableEvent) {
          ...
          if (dragMeta.create) {
            const addingEvents = internal.eventTupleToStore(droppableEvent)
            receivingContext.dispatch({
              type: 'MERGE_EVENTS',
      ...
      const hitDragging = this.hitDragging = new HitDragging(dragging, internal.interactionSettingsStore)
  ...
  function computeEventForDateSpan (dateSpan, dragMeta, context) {
    ...
    const { refined, extra } = internal.refineEventDef(defProps, context)
    const def = internal.parseEventDef(refined, extra, dragMeta.sourceId, dateSpan.allDay, context.options.forceEventDuration || Boolean(dragMeta.duration), // hasEnd
      context)
    ...
    const instance = internal.createEventInstance(def.defId, { start, end })
    return { def, instance }
  }
  ...
  function getEmbeddedElData (el, name) {
    const prefix = internal.config.dataAttrPrefix
