...
FullCalendar.RRule = (function (exports, core, rrule, internal) {
  ...
  const recurringType = {
    parse (eventProps, dateEnv) {
      if (eventProps.rrule != null) {
        const eventRRuleData = parseEventRRule(eventProps, dateEnv)
        if (eventRRuleData) {
          return {
  ...
  function parseEventRRule (eventProps, dateEnv) {
    ...
    if (typeof eventProps.rrule === 'string') {
      const res = parseRRuleString(eventProps.rrule)
    ...
    if (typeof eventProps.rrule === 'object' && eventProps.rrule) { // non-null object
      const res = parseRRuleObject(eventProps.rrule, dateEnv)
    ...
    const exdateInputs = [].concat(eventProps.exdate || [])
    const exruleInputs = [].concat(eventProps.exrule || [])
    for (const exdateInput of exdateInputs) {
      const res = internal.parseMarker(exdateInput)
    ...
    for (const exruleInput of exruleInputs) {
      const res = parseRRuleObject(exruleInput, dateEnv)
  ...
  function parseRRuleObject (rruleInput, dateEnv) {
    ...
    function processDateInput (dateInput) {
      if (typeof dateInput === 'string') {
        const markerData = internal.parseMarker(dateInput)
        if (markerData) {
    ...
    const rruleOptions = Object.assign(Object.assign({}, rruleInput), {
  ...
  function parseRRuleString (str) {
    const rruleSet = rrule.rrulestr(str, { forceset: true })
    const analysis = analyzeRRuleString(str)
  ...
  function analyzeRRuleString (str) {
    ...
    function processMatch (whole, introPart, datePart) {
      const result = internal.parseMarker(datePart)
